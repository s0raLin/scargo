...
pub fn add_dependency(project_dir: &Path, dep_spec: &str) -> anyhow::Result<()> {
    ...
    // 步骤 1：解析为 Document 并声明为 mut
    let mut doc: Document<String> = content
        .parse()
        .context("Failed to parse Scargo.toml as TOML document")?;

    // 步骤 2：显式获取根 Table 的可变引用 (&mut Table)
    // 我们使用 DerefMut trait 提供的 deref_mut 方法，这是获取内部 Table 的标准、显式方式。
    let root_table: &mut Table = doc.deref_mut();
    ...
    // 在 root_table 上操作，它是一个 &mut Table
    let deps = root_table
        .entry("dependencies")
        .or_insert(Item::Table(Table::new())); // 使用 Table::new()
